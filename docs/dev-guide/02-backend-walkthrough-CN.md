...
│
└── learn_personal_portfolio_ai/   # 核心业务逻辑
    ├── __init__.py
...
### `learn_personal_portfolio_ai/config.py` - 配置管理
...
### `learn_personal_portfolio_ai/runtime.py` - 运行时检测
...
### `learn_personal_portfolio_ai/multi_round_bedrock_runtime_chat_manager.py` - 对话管理
...
### `learn_personal_portfolio_ai/ai_sdk_adapter.py` - 格式转换
...
### `learn_personal_portfolio_ai/boto_ses.py` - AWS Client 初始化
...
### `learn_personal_portfolio_ai/prompts/` - Prompt 模板

| 文件 | 用途 |
|------|------|
| `instruction.md` | System Prompt，定义 AI 的行为和角色 |
| `knowledge-base.md` | 知识库，包含 AI 应该参考的背景信息（个人简历、项目经验等） |

这些文件通过 `paths.py` 加载，在 `api/index.py` 中注入到 Bedrock 会话。
...
api/index.py
    ...
    ↓ 加载 config
    ↓ 初始化 ChatSession
    ↓ 注入 System Prompt + 知识库
    ↓ 转换消息格式 (ai_sdk_adapter)
...
| 需求 | 修改位置 |
|------|----------|
| 换 AI 模型 | `config.py` 的 `model_id` |
| 改 System Prompt | `prompts/instruction.md` |
| 更新知识库 | `prompts/knowledge-base.md` |
